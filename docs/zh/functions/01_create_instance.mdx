...
在创建 MySQL-PXC 实例之前，确保您已经查看过<ExternalSiteLink name="acp" href="/appservice/overview/features#compatible_sc" children="支持的存储类" />，并确认当前集群支持该类型的存储类且有足够的可用空间。建议使用 TopoLVM 存储类。
